...
**CoreWorkPal** 是一款基于 **Tauri 2 + React 19 + TypeScript + Rust** 开发的轻量级桌面硬件诊断与趣味工坊养成伴侣。项目将核心硬件指标（处理器、显卡、内存、网络、温度、磁盘）与复古像素美术、桌面宠物动作状态机以及工坊升级体系进行趣味化融合，旨在为开发者与运维工程师提供一个精致、好玩且实用的桌面副屏伴侣。
...
* **工坊长线策略养成**：
  ...
  * 提供实时增益面板，清晰展示当前工坊等级带来的生产速度修正（如 `等级 * 15%`）与硬件模块产出加成。
...
### 4. 🎨 复古像素视觉美学
* **全量矢量化**：工坊的 6 个工作台背景插图全部使用自研的高饱和度复古像素 SVG 矢量设计（比例为 `140x40`，完美适配 `280x80` 弹窗比例），在边缘绝对清晰锐利的同时，将包体积从原来的约 `6MB` 降至 `20KB` 以下。
